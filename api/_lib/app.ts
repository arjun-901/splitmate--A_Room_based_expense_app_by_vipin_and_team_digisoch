...
app.get("/api/health", async (_req, res) => {
  try {
    await ensureDBConnected();
    return res.json({ ok: true, dbName: DB_NAME });
  } catch (error) {
...
app.get("/api/rooms/:code", authenticate, async (req: AuthenticatedRequest, res) => {
  ...
  const memberProfiles = Object.fromEntries(users.map((userDoc) => [userDoc._id.toString(), sanitizeUser(userDoc)]));
...
app.delete("/api/rooms/:code/members/:memberId", authenticate, async (req: AuthenticatedRequest, res) => {
  ...
  await db.collection("rooms").updateOne({ code: roomCode }, { $pull: { members: memberId } as any });
...
app.put("/api/expenses/:id", authenticate, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    const normalizedExpense = normalizeExpensePayload({ ...existingExpense, amount }, room.members, req.user!.id);
